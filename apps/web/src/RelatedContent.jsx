...
export function RelatedProducts({ products, onOpen, title = "Produk serupa", subtitle = "Yang lain juga lihat ini." }) {
  ...
  return (
    <section className="shell" style={{ marginBottom: 40 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(products.length, 3)}, 1fr)`, gap: 20 }}>
        {products.map(p => (
          <article key={p.id} className="card" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 12, cursor: "pointer" }} onClick={() => onOpen?.(p)}>
            <div style={{
...
export function RelatedCourses({ courses, onOpen, title = "Kelas lainnya", subtitle = "Mungkin cocok buat kamu." }) {
  ...
  return (
    <section className="shell" style={{ marginBottom: 40 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(courses.length, 3)}, 1fr)`, gap: 20 }}>
        {courses.map(k => (
          <article key={k.id} className="card" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 12, cursor: "pointer" }} onClick={() => onOpen?.(k)}>
            <div style={{
